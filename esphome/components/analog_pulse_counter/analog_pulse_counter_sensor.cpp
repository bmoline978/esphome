#include "analog_pulse_counter_sensor.h"
...
static const char *const TAG = "analog_pulse_counter";
...
void AnalogPulseCounterSensor::setup() {
  ESP_LOGCONFIG(TAG, "Setting up pulse counter '%s'...", this->name_.c_str());
  this->maxWindowSize = MAX_WINDOW_SIZE;
  this->readings = new int[this->maxWindowSize];

  for (int i = 0; i < this->maxWindowSize; i++)
    this->readings[i] = 0;
  this->windowSize = this->maxWindowSize;
  // if (!this->storage_.analog_pulse_counter_setup(this->pin_)) {
  //   this->mark_failed();
  //   return;
  // }
  high_freq_.start();
...
void AnalogPulseCounterSensor::dump_config() {
...
void AnalogPulseCounterSensor::loop() {
  // pulse_counter_t raw = 0; //this->storage_.read_raw_value();
  // uint32_t now = millis();
  ...
  // }
  // this->last_time_ = now;
  if (micros() - LoopStartMicros >= 600) {    // Sample every 600 usec
    LoopStartMicros = micros();

    int currentReading = analogRead(this->pin_->get_pin());;
    if (startTime == 0)
      startTime = micros();

    // Calculate moving average
    accumulator -= readings[readingIndex];      //Remove the oldest entry from the sum
    readings[readingIndex] = currentReading;           // Add the newest reading to the window
    accumulator += currentReading;                 // Add the newest reading to the sum

    readingIndex = ++readingIndex % windowSize;   // Increment the index, and wrap to 0 if it exceeds the window size
    readingCount = std::min(++readingCount, windowSize);
    bool sampleIsValid = (readingCount == windowSize);

    signalFloor =   accumulator / readingCount; // Use average signal for floor

    int sensorIn = currentReading - signalFloor;

    if (sampleIsValid) {
      maxIn = std::max(sensorIn, maxIn);
      if (sensorIn >= pulseThreshold) {
        if ((prevIn < pulseThreshold) && (countLow > 1)) { // Do not reset pulse count if only one reading drops below threshold
          //                                                  pulse was low for more than one + one measurement and is just high
          PulseStartMicros = micros();
          countLowOld = countLow;
          countLow = 0;
          firstIn = sensorIn;
        }
        countHigh++;
        minIn = std::min(sensorIn, minIn);
      }
      else {
        if (countHigh > 1) {  // Do not reset pulse count if only one reading drops below threshold
          pulseWidth = micros() - PulseStartMicros;
          //                                 pulse is now low and was high for 1 + 1 measurement
          countHighOld = countHigh;
          countHigh = 0;
          // *********************************  have a valid pulse  !!!!!!!!!!!!!!!!!!!!!!!!!!!
          if (prevPulseEndMicros != 0 ) {
            watts = 3600000 / ((micros() - prevPulseEndMicros) / 1000);
            cumWH++;
          }

          prevPulseEndMicros = micros();
    ...
    prevIn = sensorIn;
    if ((++sampleCount % 3333) == 0) { // Publish every 2 seconds
      sampleCount = 0;
      float value = watts / 60;  // per minute
      this->publish_state(value);
      if (this->total_sensor_ != nullptr) 
        this->total_sensor_->publish_state(cumWH);
      
      ESP_LOGD("APULSE", "watts = %f", watts);
      ESP_LOGD("APULSE", "rawSensorIn: %d ", currentReading);
      ESP_LOGD("APULSE", "signalFloor: %d", signalFloor);
      ESP_LOGD("APULSE", "pulseWidth: %d", pulseWidth);
      ESP_LOGD("APULSE", "maxInOld: %d", maxInOld);
    }
  }
}
